...
write_runtime_profiles_yaml() {
  ...
  # Determine profile name based on environment
  case "${ENV_NAME}" in
    test)       
      PROFILE_NAME="dbt-test" 
      log "✅ Using profile: dbt-test"
      ;;
    staging)    
      PROFILE_NAME="dbt-staging"
      log "✅ Using profile: dbt-staging"
      ;;
    production) 
      PROFILE_NAME="dbt-production"
      log "✅ Using profile: dbt-production"
      ;;
    *)          
...
YAML
  ...
  log "✅ DBT_PROFILE=${PROFILE_NAME} set in:"
...
main() {
  ...
  fetch_redshift_secrets_to_envfile
  write_runtime_profiles_yaml  # This now sets DBT_PROFILE
  prepare_deployment_directory
  prepare_dbt_host_logging_dirs
